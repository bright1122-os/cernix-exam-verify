...
<style>
    ...
    .ex-user-info span { font-size: 11px; color: rgba(255,255,255,.5); }
    .ex-logout {
        width: 36px;
        height: 36px;
        border-radius: 10px;
    ...
    .ex-logout:hover { background: rgba(255,255,255,.12); color: #fff; }
    ...
    .camera-feed {
        ...
        inset: 0;
        background:
            radial-gradient(circle at 50% 40%, rgba(45,108,255,.12), transparent 60%),
    ...
    .last-scan .dot { width: 7px; height: 7px; border-radius: 50%; background: rgba(255,255,255,.25); }
    ...
    .last-scan .info span { font-size: 11px; color: rgba(255,255,255,.5); }
    .last-scan .time { font-size: 11px; color: rgba(255,255,255,.45); font-family: 'JetBrains Mono', monospace; }

    /* ── Takeovers (mobile only full-screen) ─────────────────────────────── */
    .takeover {
        position: absolute;
        inset: 0;
        display: none;
    ...
    .takeover::before {
        ...
        position: absolute;
        inset: 0;
        background: radial-gradient(circle at 50% 0%, rgba(255,255,255,.13), transparent 60%);
    }
    .takeover .to-top {
    ...
    .takeover .to-top .status-label {
        font-size: 10px;
        font-weight: 700;
        letter-spacing: .3em;
        opacity: .7;
    }
    .takeover .to-top .time {
        font-family: 'JetBrains Mono', monospace;
        font-size: 11px;
        opacity: .7;
    }
    ...
    .takeover .big-icon svg { width: 60px; height: 60px; stroke: #fff; stroke-width: 3; fill: none; }
    .takeover h1 { font-size: 48px; font-weight: 800; letter-spacing: .04em; margin: 0; line-height: .95; }
    .takeover p { font-size: 15px; margin: 10px 0 0; opacity: .8; }
    .student-card {
        margin: 20px 20px 0;
        padding: 16px;
        background: rgba(255,255,255,.12);
    ...
    .student-card .mt { font-size: 12px; opacity: .65; margin: 2px 0 0; font-family: 'JetBrains Mono', monospace; }
    .student-card .dept { font-size: 11px; opacity: .55; margin: 4px 0 0; text-transform: uppercase; letter-spacing: .08em; }
    .meta-row {
        margin: 12px 20px 0;
        display: grid;
    ...
    .meta-cell .k { font-size: 9px; opacity: .55; letter-spacing: .1em; text-transform: uppercase; }
    ...
    .takeover .to-bottom button.primary {
        background: #fff;
        color: #065f46;
        border-color: #fff;
    }
    ...
    .takeover .to-bottom button:hover { filter: brightness(.94); }
    ...
    .res-student-card .mt { font-size: 12px; color: rgba(255,255,255,.55); margin: 2px 0 0; font-family: 'JetBrains Mono', monospace; }
    .res-student-card .dept { font-size: 11px; color: rgba(255,255,255,.4); margin: 4px 0 0; text-transform: uppercase; letter-spacing: .08em; }
    ...
    .res-meta-cell .k { font-size: 9px; color: rgba(255,255,255,.4); letter-spacing: .1em; text-transform: uppercase; }
    ...
    .res-rejection-block .v { font-size: 13px; color: rgba(255,255,255,.85); font-weight: 600; }
    .res-rejection-block .sub { font-size: 11px; color: rgba(255,255,255,.5); margin-top: 2px; }
...
<div class="ex-page">
    ...
    <div class="ex-topbar">
        <div class="ex-brand">
            ...
            <div>
                <b>CERNIX</b>
                <span style="display:block;font-size:10px;letter-spacing:.06em;color:rgba(255,255,255,.5)">EXAM VERIFICATION</span>
            </div>
        </div>
        <div class="ex-user">
            <div class="ex-user-info" style="text-align:right;display:none;gap:4px" style="max-width:160px">
                <b>{{ $examiner['full_name'] ?? 'Examiner' }}</b>
                <span style="font-size:11px">{{ '@' . ($examiner['username'] ?? 'examiner') }} · {{ ucfirst($examiner['role'] ?? 'examiner') }}</span>
            </div>
    ...
    <div class="ex-workspace">
        ...
        <div class="ex-camera-panel">
            ...
            <!-- Mobile takeovers (hidden on desktop) -->
        ...
        <!-- Desktop result panel -->
        <div class="ex-result-panel" id="result-panel">
            <!-- Idle state -->
            <div class="res-idle" id="res-idle">
                <div class="idle-icon">
                    <svg width="32" height="32" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" style="color:rgba(255,255,255,.3)">
                        <rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><path d="M14 14h.01M14 17h.01M17 14h.01M17 17h.01M17 20h.01M20 14h.01M20 17h.01M20 20h.01"/>
                    </svg>
                </div>
                <b>Ready to scan</b>
                <p>Point the camera at a student's CERNIX QR code.<br>Results appear here instantly.</p>
            </div>
            ...
            <div class="res-result" id="res-result">
                <div class="res-status-bar">
                    ...
                    <div style="font-size:11px;color:rgba(255,255,255,.4);margin-top:8px;font-family:'JetBrains Mono',monospace" id="res-timestamp"></div>
                </div>

                <div class="res-student-block">
                    <!-- Approved: student card + meta -->
                ...
                <div class="res-result-actions" id="res-result-actions-approved">
...
function updateDesktopResult(type){
    ...
    if(type==='approved'){
        ...
        actionBtn.innerHTML=type==='rejected'?'Alert Supervisor':'View Audit';
    }
}
...
async function handleQRCode(rawData){
    ...
    document.getElementById('scan-prompt').textContent='Verifying…';
    try{
